...
for i in range(1, 11):
    filename = "./data/personal_lightfield/dresser" + str(i) + '.JPG'
    ...
    # L = np.transpose(im.reshape(16, 375, 16, 250, 3)).astype(np.uint8)
    L = np.transpose(im.reshape(16, 16, 250, 375, 3, order='F'), (0, 1, 2, 3, 4)).astype(np.uint8)

    # print("Shape of L:", L.shape)
    photo_stack.append(L)

fs = np.array(photo_stack)
# # # print("Shape of fs:", fs.shape)
# # fs = fs[1:]
# fs = np.transpose(fs.reshape(16, 16, 250, 375, 3, order='F'), (0, 1, 2, 3, 4)).astype(np.uint8)
# fs = np.concatenate(photo_stack, axis=0).reshape((16, 16, 250, 375, -1))
...
ds = [-2.0, -1.5, -1.0, -0.5, 0.0, 0.5, 1.0, 1.5, 2.0, 2.5]
# for i in range(len(ds)):
#     r = refocus(fs[i], ds[i])
#     fig = plt.figure()
#     plt.imshow(r)
#     plt.title(f'Depth={ds[i]}')
#     plt.savefig(f'personal_lf_results/depth{i + 1}.png')
...
focused_image, depth_map = allInFocus(fs)
